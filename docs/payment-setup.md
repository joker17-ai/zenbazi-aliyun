...
已部署统一收银台。服务器使用官方 Node 22.23.2（下载时校验 SHA-256），由 systemd 的 `runtime.conf` 指定独立安装路径。该配置还加载可选的 `/opt/zenbazi/payment.env`。原数据库环境文件继续保留，支付凭证密钥已生成；无需在商户文件中再填写数据库或支付凭证密钥。

PostgreSQL 的本机 TCP 连接已从临时 trust 恢复为 SCRAM 密码认证，并验证数据库连接正常。应用备份保存在 `/opt/backups/zenbazi-checkout-20260910/`。当前商户资料仍未配置，真实支付验收未完成。
